...
def init_distributed_mode(args):
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                        ...
                                        #  pg_options=torch.distributed.ProcessGroupNCCL.Options(device_ids=[args.gpu])
                                         )
...
def vis2tb(output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(output_dict["disp_l1"][-1], gt_disp, mode="bilinear") * width
        ...
        ## Optical Flow Eval
        ##################################################
        if 'flow_f' in output_dict:
            out_sceneflow = interpolate2d_as(output_dict['flow_f'][-1], gt_flow, mode="bilinear")
        else:
            out_sceneflow = interpolate2d_as(output_dict['flows_f12'][-1], gt_flow, mode="bilinear")
        ...
        return output_dict
def vis2tb_my_aug(flow, disp, target_dict):
         
        gt_disp = torch.ones(1,1,375,1242)
        gt_flow = torch.ones(1,2,375,1242)
        batch_size, _, _, width = gt_disp.size()

        out_disp_l1 = interpolate2d_as(disp, gt_disp, mode="bilinear") * width
        
        out_sceneflow = interpolate2d_as(flow, gt_flow, mode="bilinear")
        out_flow, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], out_sceneflow, out_disp_l1)

        return out_flow, out_disp_l1

def log(writer, outputs, losses, target, recons_imgs, n_iter, mode):
    if recons_imgs is None:
        return
