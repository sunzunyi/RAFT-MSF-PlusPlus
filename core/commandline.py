...
def _add_arguments_for_module(parser,
    ...
    # -------------------------------------------------------------------------
    module_dict = tools.module_classes_to_dict( # returns a dictionary of class names and constructors
    ...
    # class_constructor:  <class 'torch.optim.adam.Adam'>
...
def _parse_arguments():
    ...
    add("--start_occ_reg", type=int, default=10000)
    add("--sequence_length", type=int, default=3)

    add("--detector", default="YOLOv5", help="detector model to use")
    add("--detector_confidence_threshold", default=0.5, type=float, help="confidence threshold for the detector")
    add("--detector_nms_threshold", default=0.4, type=float, help="NMS threshold for the detector")
    add("--detector_input_size", default=640, type=int, help="input size for the detector")
    add("--detector_classes", default=80, type=int, help="number of classes for the detector")
    ...
    _add_arguments_for_module(
        ...
        exclude_classes=["_*"],
        exclude_params=["self","args"],
        forced_default_types={"imgsize": tools.str2intlist} )
    ...
    _add_arguments_for_module(
        ...
        exclude_classes=["_*"],
        unknown_default_types={"root": str, "root_home": str, "root_raw": str})
    ...
    _add_arguments_for_module(
        ...
        unknown_default_types={"T_max": int,
                               ...
                               "gamma": float,
                               'max_lr': float,},
        forced_default_types={"steps_per_epoch": int,
                            "epochs": int,
                              'total_steps': int,
                              })                       
...
def setup_logging_and_parse_arguments(blocktitle):
    ...
    logger.configure_logging(os.path.join(args.save, 'logbook.txt'), is_main_process)
    # ----------------------------------------------------------------------------
    ...
    with logger.LoggingBlock(blocktitle, emph=True):
        for argument, value in sorted(vars(args).items()):
            ...
            logger.myinfo('{}{}: {}{}'.format(color, argument, value, reset))
