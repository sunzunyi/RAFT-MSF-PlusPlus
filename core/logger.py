...
def get_timestamp():
    ...
    return (datetime.now() + timedelta(hours=8)).strftime('%H:%M')


is_main_processgs = True
def myinfo(msg):
    global is_main_processgs
    if is_main_processgs:
        logging.info(msg)
...
import time
def configure_logging(filename=None, is_master=True):
    global is_main_processgs
    is_main_processgs = is_master
    ...
    if filename is not None:
        ...
        if not os.path.exists(d) and is_master:
...
class LoggingBlock:
    def __init__(self, title, emph=False):
        ...
        if emph:
            myinfo("%s==>%s %s%s%s" % (cyan, reset, bright, title, reset))
        else:
            myinfo(title)
