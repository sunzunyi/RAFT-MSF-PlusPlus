...
class RAFT(nn.Module):
    ...
    def forward_2_frames(self, image1, image2, K, input_size):
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                net, up_mask, disp_d, sf_d = self.update_block(net, inp, corr, flow, disp, sf, attention, image1, itr)
            disp = disp + disp_d
            sf = sf + sf_d
        ...
        return sf_predictions, [],  [],  [], disp_predictions, []
    ...
    def forward_4_frames(self, img0, img1, img2, img3, K, input_size):
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                net_f, up_mask_f, disp_d_f, sf_f_d = self.update_block(net_f, inp_f, corr_f, flow_f, disp_f, sf_f, attention)
                net_b, up_mask_b, disp_d_b, sf_b_d = self.update_block(net_b, inp_b, corr_b, flow_b, disp_b, sf_b, attention)
    ...
    def forward(self, input_dict):
        ...
        if self.args.evaluation or self.args.finetuning or (self.args.eval_on_train and not self.training):
            ## Left
            output_dict['flows_f12'], output_dict['flows_f23'], output_dict['flows_b10'], output_dict['flows_b21'], \
            output_dict['disp_l1'], output_dict['disp_l2'] = self.forward_2_frames(input_dict['input_l1_aug'], input_dict['input_l2_aug'], k1, input_size)
            ...
            flows_f12, flows_f23, flows_b10, flows_b21, disp_l1, disp_l2 = self.forward_2_frames(input_l1_flip, input_l2_flip, k_l1_flip, input_size)
