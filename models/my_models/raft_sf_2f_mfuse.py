...
from models.update import BasicUpdateBlock, conv
from models.my_models.update import MFUSE
class RAFT(nn.Module):
    ...
    def forward_4_frames(self, img0, img1, img2, img3, K, input_size, return_feat = False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            disp_f = self.disp_head(fmaps[:, 1:3, ...].reshape(b*2, -1, h, w))
            disp_b = disp_f.clone()
        coords0_f = coords_grid(B*2, H//8, W//8).to(img0.device)
        coords0_b = coords_grid(B*2, H//8, W//8).to(img0.device)
        _, coords1_f = self.sceneF2opticalF(disp_f * 0.3, sf_f, K, input_size) # multiplied by 0.3: ijcvRevision
        _, coords1_b = self.sceneF2opticalF(disp_b * 0.3, sf_b, K, input_size) # multiplied by 0.3: ijcvRevision
        ...
        for itr in range(iters):
            ...
            disp_f = disp_f.detach()
            disp_b = disp_b.detach()
            sf_f = sf_f.detach()
            sf_b = sf_b.detach()
            ...
            with autocast(enabled=self.args.mixed_precision):
                net_f, up_mask_f, disp_d_f, sf_f_d = self.update_block(net_f, inp_f, corr_f, flow_f, disp_f, sf_f)
                net_b, up_mask_b, disp_d_b, sf_b_d = self.update_block(net_b, inp_b, corr_b, flow_b, disp_b, sf_b)
                
            disp_f = disp_f + disp_d_f
            disp_b = disp_b + disp_d_b
            sf_f = sf_f + sf_f_d
            sf_b = sf_b + sf_b_d
            _, coords1_f = self.sceneF2opticalF(self.sigmoid(disp_f) * 0.3, sf_f, K, input_size) 
            _, coords1_b = self.sceneF2opticalF(self.sigmoid(disp_b) * 0.3, sf_b, K, input_size)
            
            disp_f_up = self.cvx_up_pred(self.sigmoid(disp_f) * 0.3, up_mask_f)
            disp_b_up = self.cvx_up_pred(self.sigmoid(disp_b) * 0.3, up_mask_b)
            sf_f_up = self.cvx_up_pred(sf_f, up_mask_f)
            sf_b_up = self.cvx_up_pred(sf_b, up_mask_b)

            disp_f_up = disp_f_up.reshape(b, 2, -1, H, W)
            disp_b_up = disp_b_up.reshape(b, 2, -1, H, W)
            disp1_predictions.append(disp_f_up[:, 0, ...])
            disp2_predictions.append(disp_b_up[:, 1, ...])
        ...
        if return_feat:
            corr_up_f = self.cvx_up_pred(corr_f[:, [24], ...], up_mask_f)  # B*2, 1, H, W
            corr_up_b = self.cvx_up_pred(corr_b[:, [24], ...], up_mask_b)
            return flows_f12, flows_f23, flows_b10, flows_b21, disp1_predictions, disp2_predictions, \
                    corr_up_f.reshape(b, 2, -1, H, W)[:, 0, ...], corr_up_f.reshape(b, 2, -1, H, W)[:, 1, ...], \
                    corr_up_b.reshape(b, 2, -1, H, W)[:, 0, ...], corr_up_b.reshape(b, 2, -1, H, W)[:, 1, ...]
...
class MFUSE_RAFT(nn.Module):
    def __init__(self, args, freeze_baseline = True):
        super(MFUSE_RAFT, self).__init__()
        self.args = args
        self.raft = RAFT(args)
        # state_dict = torch.load('/raft-msf/ckpt/latest.ckpt')["state_dict"]   # according to mfuse's training plan
        # for key in list(state_dict.keys()):
        #     if key.startswith('_model.'):
        #         # remove `module.`
        #         state_dict[key[len("_model.module."):]] = state_dict[key]
        #         del state_dict[key]
        # self.raft.load_state_dict(state_dict, strict=True)
        self.mfuse = MFUSE(in_channels= 9)
        # if freeze_baseline:
        #     for param in self.raft.parameters():
        #         param.requires_grad = False

    def forward_4_frames(self, img0, img1, img2, img3, K, input_size):
        flows_f12, flows_f23, flows_b10, flows_b21, \
        disp1_predictions, disp2_predictions, \
        corr_up_f12, corr_up_f23,  corr_up_b10, corr_up_b21= self.raft.forward_4_frames(img0, img1, img2, img3, K, input_size, return_feat=True)
        F12, D1_f = self.mfuse(torch.cat((flows_f12[-1], flows_b10[-1], disp1_predictions[-1], corr_up_f12, corr_up_b10), dim=1))
        F10, D1_b = self.mfuse(torch.cat((flows_b10[-1], flows_f12[-1], disp1_predictions[-1], corr_up_b10, corr_up_f12), dim=1))
        # D1 = (D1_f + D1_b) / 2.0
        F23, D2_f = self.mfuse(torch.cat((flows_f23[-1], flows_b21[-1], disp2_predictions[-1], corr_up_f23, corr_up_b21), dim=1))
        F21, D2_b = self.mfuse(torch.cat((flows_b21[-1], flows_f23[-1], disp2_predictions[-1], corr_up_b21, corr_up_f23), dim=1))
        # D2 = (D2_f + D2_b) / 2.0
        flows_f12.append(F12)
        flows_b10.append(F10)
        flows_f23.append(F23)
        flows_b21.append(F21)
        disp1_predictions.append(disp1_predictions[-1])
        disp2_predictions.append(disp2_predictions[-1])
        return flows_f12, flows_f23, flows_b10, flows_b21, disp1_predictions, disp2_predictions
