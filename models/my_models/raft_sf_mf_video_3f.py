...
from models.my_models.update import BasicUpdateBlock, conv, BasicUpdateBlock_video_3f, initialize_msra
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        self.update_block = BasicUpdateBlock_video_3f(self.args, hidden_dim=hdim)
    ...
    # Although the input is 4 frames, it is actually only 3 frames, namely the first three frames and the last three frames
    def forward_4_frames(self, img0, img1, img2, img3, K, input_size):
        ...
        # print('coords0_f:', coords0_f.shape)
        ...
        # if torch.isnan(coords1_f).any() or torch.isnan(coords1_b).any():
        #     assert False, "coords1_f or coords1_b has nan"
        disp1_predictions = []
        disp2_predictions = []
        flows_f12 = []
        flows_f23 = []
        ...
        for itr in range(iters):
            ...
            corr = torch.cat((corr_f, corr_b), dim=1)
            ...
            with autocast(enabled=self.args.mixed_precision):
                net, up_mask, disp_d, delta_flow = self.update_block(net, inp, corr, sf, disp, flow)
