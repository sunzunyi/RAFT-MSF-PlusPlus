...
from models.my_models.update import BasicUpdateBlock_corr, conv
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        self.update_block = BasicUpdateBlock_corr(self.args, hidden_dim=hdim, with_mhs = True)
    ...
    def forward_4_frames(self, img0, img1, img2, img3, K, input_size):
        ...
        # sf_f_mhs, sf_b_mhs = self.init_mhs_fb.repeat(b, 4, 1, h, w).split(2, dim=1) # [:2]
        ...
        for itr in range(self.args.iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                net_f, up_mask_f, disp_d_f, sf_f_d, forward_motion_hidden_state = self.update_block(net_f, inp_f, corr_f, corr_b, 
                                                                                                    flow_f, flow_b, disp, sf_f, sf_b,
                                                                                                    forward_12_23.reshape(b*2, -1, h, w), 
                                                                                                    forward_motion_hidden_state.reshape(b*2, -1, h, w)
                                                                                                    )
                net_b, up_mask_b, disp_d_b, sf_b_d, backward_motion_hidden_state = self.update_block(net_b, inp_b, corr_b, corr_f,
                                                                                                    flow_b, flow_f, disp, sf_b, sf_f,
