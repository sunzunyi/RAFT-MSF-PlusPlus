...
def pts2pixel(pts, intrinsics):
    ...
    pixels_mat = proj_pts.div(proj_pts[:, 2:3, :] + 1e-6)[:, 0:2, :]
...
from utils.interpolation import interpolate2d, get_coordgrid
...
def sceneF2opticalF(disp, sf, K, input_size):
    _, _, h, w = disp.size()
    disp = disp * w
    local_scale = torch.zeros_like(input_size)
    local_scale[:, 0] = h
