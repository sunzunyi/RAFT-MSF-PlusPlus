...
class Augmentation_ScaleCrop(nn.Module):
    ...
    def augment_intrinsic_matrices(self, intrinsics, num_splits, img_size, resize, params):
        ...
        intm_size_h = torch.floor(img_size[0] * intm_scale)
        intm_size_w = torch.floor(img_size[1] * intm_scale)
        # print("intm_size_h, intm_size_w", intm_size_h, intm_size_w) # 356 1203   354   1171
        scale_x = intm_size_w / resize[1]
        scale_y = intm_size_h / resize[0]
...
class Augmentation_SceneFlow(Augmentation_ScaleCrop):
    ...
    def forward(self, example_dict):
        ...
        if 'input_sam_fullseg_l1' in example_dict:
            sam_fullseg1 = example_dict["input_sam_fullseg_l1"]
            sam_fullseg1 = my_grid_sample(sam_fullseg1, coords)
            example_dict["input_sam_fullseg_l1_aug"] = sam_fullseg1

            sam_fullseg2 = example_dict["input_sam_fullseg_l2"]
            sam_fullseg2 = my_grid_sample(sam_fullseg2, coords)
            example_dict["input_sam_fullseg_l2_aug"] = sam_fullseg2
...
class Augmentation_SceneFlow_MF(Augmentation_ScaleCrop):
    def __init__(self, args, photometric=True, trans=0.07, scale=[0.93, 1.0], resize=[256, 832]):   #    [256, 832]    [128, 416]
        super(Augmentation_SceneFlow_MF, self).__init__(
            args, 
            photometric=photometric, 
            trans=trans, 
            scale=scale, 
    ...
    def forward(self, example_dict):
        ...
        im_l1 = example_dict["input_l1"]
        im_l2 = example_dict["input_l2"]
        im_l3 = example_dict["input_l3"]
        im_r0 = example_dict["input_r0"]
        im_r1 = example_dict["input_r1"]
        im_r2 = example_dict["input_r2"]
        im_r3 = example_dict["input_r3"]
        ...
        ## Augment images
        im_l0 = my_grid_sample(im_l0, coords)
        im_l1 = my_grid_sample(im_l1, coords)
        im_l2 = my_grid_sample(im_l2, coords)
        im_l3 = my_grid_sample(im_l3, coords)
        im_r0 = my_grid_sample(im_r0, coords)
        im_r1 = my_grid_sample(im_r1, coords)        
        im_r2 = my_grid_sample(im_r2, coords)
        im_r3 = my_grid_sample(im_r3, coords)


        example_dict["input_l1_no_aug"] = im_l1.clone()
        example_dict["input_l2_no_aug"] = im_l2.clone()
        example_dict["input_r1_no_aug"] = im_r1.clone()
        example_dict["input_r2_no_aug"] = im_r2.clone()


        if 'input_sam_fullseg_l1' in example_dict:
            sam_fullseg1 = example_dict["input_sam_fullseg_l1"]
            sam_fullseg1 = tf.grid_sample(sam_fullseg1, coords, mode='nearest', align_corners=True)
            example_dict["input_sam_fullseg_l1_aug"] = sam_fullseg1

            sam_fullseg2 = example_dict["input_sam_fullseg_l2"]
            sam_fullseg2 = tf.grid_sample(sam_fullseg2, coords, mode='nearest', align_corners=True)
            example_dict["input_sam_fullseg_l2_aug"] = sam_fullseg2
        ...
        if self._photometric and torch.rand(1) > 0.5:
            im_left = torch.stack([im_l0, im_l1, im_l2, im_l3], dim=1) # b 4 c H W
            im_left = im_left.permute(0,2,3,4,1).reshape(self._batch, 3, self._resize[0], self._resize[1] * 4)
            im_right = torch.stack([im_r0, im_r1, im_r2, im_r3], dim=1) # b 4 c H W
            im_right = im_right.permute(0,2,3,4,1).reshape(self._batch, 3, self._resize[0], self._resize[1] * 4)

            im_left, im_right = self._photo_augmentation(im_left, im_right)
            im_left = im_left.reshape(self._batch, 3, self._resize[0], self._resize[1], 4).permute(0,4,1,2,3).contiguous()
            im_l0, im_l1, im_l2, im_l3 = torch.chunk(im_left, 4, dim=1)
            im_l0 = im_l0.squeeze_(1)
            im_l1 = im_l1.squeeze_(1)
            im_l2 = im_l2.squeeze_(1)
            im_l3 = im_l3.squeeze_(1)
            im_right = im_right.reshape(self._batch, 3, self._resize[0], self._resize[1], 4).permute(0,4,1,2,3).contiguous()
            im_r0, im_r1, im_r2, im_r3 = torch.chunk(im_right, 4, dim=1)
            im_r0 = im_r0.squeeze_(1)
            im_r1 = im_r1.squeeze_(1)
            im_r2 = im_r2.squeeze_(1)
            im_r3 = im_r3.squeeze_(1)
        ...
        example_dict["input_l1_aug"] = im_l1
        example_dict["input_l2_aug"] = im_l2
        example_dict["input_l3_aug"] = im_l3
        example_dict["input_r0_aug"] = im_r0
        example_dict["input_r1_aug"] = im_r1
        example_dict["input_r2_aug"] = im_r2
        example_dict["input_r3_aug"] = im_r3
...
## Only for finetuning. Because the sparse GT cannot be interpolated, we just use cropping
class Augmentation_SceneFlow_Finetuning_MV(nn.Module):
    def __init__(self, args, photometric=True, resize=[192, 640]):
        super(Augmentation_SceneFlow_Finetuning_MV, self).__init__()
    ...
    def kitti_random_crop(self, example_dict):
        ...
        ## Cropping
        example_dict["input_l0"] = self.cropping(example_dict["input_l0"], str_x, str_y, end_x, end_y)
        example_dict["input_l1"] = self.cropping(example_dict["input_l1"], str_x, str_y, end_x, end_y)
        example_dict["input_l2"] = self.cropping(example_dict["input_l2"], str_x, str_y, end_x, end_y)
        example_dict["input_l3"] = self.cropping(example_dict["input_l3"], str_x, str_y, end_x, end_y)
        example_dict["input_r0"] = self.cropping(example_dict["input_r0"], str_x, str_y, end_x, end_y)
        example_dict["input_r1"] = self.cropping(example_dict["input_r1"], str_x, str_y, end_x, end_y)
        example_dict["input_r2"] = self.cropping(example_dict["input_r2"], str_x, str_y, end_x, end_y)
        example_dict["input_r3"] = self.cropping(example_dict["input_r3"], str_x, str_y, end_x, end_y)
    ...
    def forward(self, example_dict):
        ...
        # Image resizing
        im_l0 = interpolate2d(example_dict["input_l0"], self._resize)
        im_l1 = interpolate2d(example_dict["input_l1"], self._resize)
        im_l2 = interpolate2d(example_dict["input_l2"], self._resize)
        im_l3 = interpolate2d(example_dict["input_l3"], self._resize)
        im_r0 = interpolate2d(example_dict["input_r0"], self._resize)
        im_r1 = interpolate2d(example_dict["input_r1"], self._resize)
        im_r2 = interpolate2d(example_dict["input_r2"], self._resize)
        im_r3 = interpolate2d(example_dict["input_r3"], self._resize)
        self._batch, _, h_orig, w_orig = im_l1.size()

        if 'input_sam_fullseg_l1' in example_dict:
            sam_fullseg1 = example_dict["input_sam_fullseg_l1"]
            sam_fullseg1 = F.interpolate(sam_fullseg1, self._resize, mode='nearest')
            example_dict["input_sam_fullseg_l1_aug"] = sam_fullseg1

            sam_fullseg2 = example_dict["input_sam_fullseg_l2"]
            sam_fullseg2 = F.interpolate(sam_fullseg2, self._resize, mode='nearest')
            example_dict["input_sam_fullseg_l2_aug"] = sam_fullseg2

        # Focal length rescaling
        _, _, hh, ww = example_dict["input_l1"].size()
        sy = self._resize[0] / hh
        sx = self._resize[1] / ww
...
class Augmentation_Resize_Only_MV(nn.Module):
    def __init__(self, args, photometric=False, imgsize=[256, 832]):  #   [256, 832]    [128, 416]
        super(Augmentation_Resize_Only_MV, self).__init__()
    ...
    def forward(self, example_dict):
        ...
        # Image resizing
        im_l0 = interpolate2d(example_dict["input_l0"], self._imgsize)
        im_l1 = interpolate2d(example_dict["input_l1"], self._imgsize)
        im_l2 = interpolate2d(example_dict["input_l2"], self._imgsize)
        im_l3 = interpolate2d(example_dict["input_l3"], self._imgsize)
        k_l1 = _intrinsic_scale(example_dict["input_k_l1"], sx, sy)
        k_l2 = _intrinsic_scale(example_dict["input_k_l2"], sx, sy)

        if self._isRight:
            im_r0 = interpolate2d(example_dict["input_r0"], self._imgsize)
            im_r1 = interpolate2d(example_dict["input_r1"], self._imgsize)
            im_r2 = interpolate2d(example_dict["input_r2"], self._imgsize)
            im_r3 = interpolate2d(example_dict["input_r3"], self._imgsize)
            k_r1 = _intrinsic_scale(example_dict["input_k_r1"], sx, sy)
            k_r2 = _intrinsic_scale(example_dict["input_k_r2"], sx, sy)
        ...
        # if self._photometric and torch.rand(1) > 0.5:
        #     if self._isRight:
        #         im_l0, im_l1, im_l2, im_l3, im_r0, im_r1, im_r2, im_r3 = self._photo_augmentation(im_l0, im_l1, im_l2, im_l3, im_r0, im_r1, im_r2, im_r3)
        #     else:
        #         im_l0, im_l1, im_l2, im_l3 = self._photo_augmentation(im_l0, im_l1, im_l2, im_l3)
