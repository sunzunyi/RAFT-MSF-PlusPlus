...
def _smoothness_motion_1st(sf, img, beta=1):
    sf_grad_x = _gradient_x(sf)
    sf_grad_y = _gradient_y(sf)
...
class Loss_SceneFlow_SelfSup(nn.Module):
    ...
    def sceneflow_loss(self, sf_f, sf_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
        ...
        pts2_warp, mask_f = reconstructPts(coord1, pts2)
        pts1_warp, mask_b = reconstructPts(coord2, pts1) 
        ...
        pts_diff1 = _elementwise_epe(pts1_tf, pts2_warp).mean(dim=1, keepdim=True) / (pts_norm1 + 1e-6)
        pts_diff2 = _elementwise_epe(pts2_tf, pts1_warp).mean(dim=1, keepdim=True) / (pts_norm2 + 1e-6)
        pts_diff1_aug = pts_diff1.detach()
        pts_diff2_aug = pts_diff2.detach()
        ...
        loss_3d_s = ( (_smoothness_motion_2nd(sf_f, img_l1_aug, beta=10.0) / (pts_norm1 + 1e-6)).mean() + (_smoothness_motion_2nd(sf_b, img_l2_aug, beta=10.0) / (pts_norm2 + 1e-6)).mean() ) / (2 ** ii)

        ## Loss Summnation
        sceneflow_loss = loss_im + self._sf_3d_pts * loss_pts + self._sf_3d_sm * loss_3d_s

        return sceneflow_loss, loss_im, loss_pts, loss_3d_s, img_l1_warp, img_l2_warp, occ_map_f, occ_map_b, pts_diff1_aug, pts_diff2_aug
    ...
    def forward(self, output_dict, target_dict):
        ...
        for ii in range(n_prediction):
            ...
            ## Sceneflow Loss    
            loss_sceneflow, loss_im, loss_pts, loss_3d_s, img_l1_warp, img_l2_warp, occ_map_f, occ_map_b, pts_diff1_aug, pts_diff2_aug = self.sceneflow_loss(
                                                                            sf_f, sf_b, 
                                                                            disp_l1, disp_l2,
        ...
        loss_dict = {}
        recons_imgs  = {}
        recons_imgs["img_r1_wrap"] = img_d1_wrap[-1]
        recons_imgs["img_r2_wrap"] = img_d2_wrap[-1]
        recons_imgs["img_sf1_wrap"] = img_sf1_wrap[-1]
        recons_imgs["img_sf2_wrap"] = img_sf2_wrap[-1]
        ...
        occ_dict = {}
        occ_dict["disp_occ_l1"] = disp_occ_l1
        occ_dict["disp_occ_l2"] = disp_occ_l2
        occ_dict["occ_map_f"] = occ_map_f
        occ_dict["occ_map_b"] = occ_map_b
        occ_dict['pts_diff1_aug'] = pts_diff1_aug
        occ_dict['pts_diff2_aug'] = pts_diff2_aug

        self.detaching_grad_of_outputs(output_dict['output_dict_r'])

        return loss_dict, recons_imgs, occ_dict
...
class Loss_SceneFlow_SelfSup_MF(nn.Module):
    def __init__(self, args):
        super(Loss_SceneFlow_SelfSup_MF, self).__init__()
    ...
    def depth_loss_left_img(self, disp_l, disp_r, img_l_aug, img_r_aug, ii, n_prediction, curr_epoch):
        img_r_warp = _generate_image_left(img_r_aug, disp_l)
        left_occ = _adaptive_disocc_detection_disp(disp_r).detach()
        ...
        ## Disparities smoothness
        loss_smooth = _smoothness_motion_2nd(disp_l, img_l_aug, beta=10.0).mean() / (2 ** (n_prediction - ii - 1))   # (2 ** ii)

        return loss_img + self._disp_smooth_w * loss_smooth, left_occ, img_r_warp


    def optFlow_loss(self,opt_flow_f, opt_flow_b, img_l1_aug, img_l2_aug, disp_occ_l1, disp_occ_l2, ii, n_prediction):
        occ_map_b = _adaptive_disocc_detection(opt_flow_f).detach() * disp_occ_l2
        occ_map_f = _adaptive_disocc_detection(opt_flow_b).detach() * disp_occ_l1
        ...
        loss_opFlow_s = ( (_smoothness_motion_2nd(opt_flow_f, img_l1_aug, beta=10.0)).mean() + 
                         (_smoothness_motion_2nd(opt_flow_b, img_l2_aug, beta=10.0)).mean() ) / (2 ** (n_prediction - ii - 1)) # (2 ** ii)

        opticalF_losses = loss_im + self._optFlow_smooth_w * loss_opFlow_s

        return opticalF_losses, loss_im, loss_opFlow_s, img_l2_warp, img_l1_warp

    def sceneflow_loss(self, sf_f, sf_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, \
                        k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii, n_prediction):
        ...
        flow_f, _ = projectSceneFlow2Flow(k1_scale, sf_f, disp_l1)
        flow_b, _ = projectSceneFlow2Flow(k2_scale, sf_b, disp_l2)
        occ_map_b_flow = _adaptive_disocc_detection(flow_f).detach()
        occ_map_b = occ_map_b_flow * disp_occ_l2
        occ_map_f_flow = _adaptive_disocc_detection(flow_b).detach()
        occ_map_f = occ_map_f_flow * disp_occ_l1
        ...
        ## 3D motion smoothness loss
        loss_3d_s = ( (_smoothness_motion_2nd(sf_f, img_l1_aug, beta=10.0) / (pts_norm1 + 1e-6)).mean() + 
                     (_smoothness_motion_2nd(sf_b, img_l2_aug, beta=10.0) / (pts_norm2 + 1e-6)).mean() ) / (2 ** (n_prediction - ii - 1))
    ...
    def detaching_grad_of_outputs(self, output_dict):
        ...
        #     for ii in range(0, len(output_dict[key])):
        #         output_dict[key][ii].detach_()
        for ii in range(0, len(output_dict['flows_f12'])):
            output_dict['flows_f12'][ii].detach_()
            output_dict['flows_f23'][ii].detach_()
            output_dict['flows_b21'][ii].detach_()
            output_dict['flows_b10'][ii].detach_()
            output_dict['disp_l1'][ii].detach_()
            output_dict['disp_l2'][ii].detach_()
    ...
    def forward(self, output_dict, target_dict):
        ...
        n_prediction = len(output_dict['flows_f12'])

        #for ii, (sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2) in enumerate(zip(output_dict['flow_f'], output_dict['flow_b'], output_dict['disp_l1'], output_dict['disp_l2'], disp_r1_dict, disp_r2_dict)):
        for ii in range(n_prediction):
            sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2 = output_dict['flows_f12'][ii], output_dict['flows_b21'][ii], output_dict['disp_l1'][ii], output_dict['disp_l2'][ii], disp_r1_dict[ii], disp_r2_dict[ii]
            ...
            ## Disp Loss
            loss_disp_l1, disp_occ_l1, img_r1_warp = self.depth_loss_left_img(disp_l1, disp_r1, img_l1_aug, img_r1_aug, ii, n_pred, target_dict['curr_epoch'])
            loss_disp_l2, disp_occ_l2, img_r2_warp = self.depth_loss_left_img(disp_l2, disp_r2, img_l2_aug, img_r2_aug, ii, n_pred, target_dict['curr_epoch'])
            loss_dp_sum = loss_dp_sum + (loss_disp_l1 + loss_disp_l2) * i_weight
...
class Loss_SceneFlow_SemiSupFinetune_MV(nn.Module):
    def __init__(self, args):
        super(Loss_SceneFlow_SemiSupFinetune_MV, self).__init__()        

        self._weights = [4.0, 2.0, 1.0, 1.0, 1.0]
        self._gamma = 0.8
        self._unsup_loss = Loss_SceneFlow_SelfSup_MF(args)
    ...
    def forward(self, output_dict, target_dict):
        ...
        loss_dict = {}
        # unsup_loss_dict, recons_imgs = self._unsup_loss(output_dict, target_dict)
        unsup_loss_dict, recons_imgs, occ_dict = self._unsup_loss(output_dict, target_dict)
        unsup_loss = unsup_loss_dict['total_loss']
        ...
        disp_loss = 0
        flow_loss = 0
        n_prediction = len(output_dict['flows_f12_pp'])
        for ii, sf_f in enumerate(output_dict['flows_f12_pp']):
            ...
            disp_loss = disp_loss + (disp_l1_loss + disp_l2_loss) * i_weight  # self._weights[ii]
            flow_loss = flow_loss + flow_l1_loss * i_weight  # self._weights[ii]
        ...
        total_loss = unsup_loss * u_weight + disp_loss * d_weight + flow_loss * f_weight
        loss_dict["uns"] = unsup_loss
        loss_dict["dp_l"] = disp_loss
        loss_dict["fl_l"] = flow_loss
        loss_dict["total_loss"] = total_loss
...
class Eval_SceneFlow_KITTI_Test_MV(nn.Module):
    def __init__(self):
        super(Eval_SceneFlow_KITTI_Test_MV, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(output_dict["disp_l1_pp"][-1], input_l1, mode="bilinear") * input_l1.size(3)
        out_depth_l1 = _disp2depth_kitti_K(out_disp_l1, intrinsics[:, 0, 0])
        out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
        output_dict["out_disp_l_pp"] = out_disp_l1
        ...
        ## Optical Flow Eval
        ##################################################
        out_sceneflow = interpolate2d_as(output_dict['flows_f12_pp'][-1], input_l1, mode="bilinear")
        out_flow, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], out_sceneflow, output_dict["out_disp_l_pp"])        
        output_dict["out_flow_pp"] = out_flow
...
class Eval_SceneFlow_KITTI_Train(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        ## 2D/3D Flow Eval
        ##################################################
        flow_gt_mag = torch.norm(target_dict["target_flow"], p=2, dim=1, keepdim=True) + 1e-6
        ## 2D Flow
...
class Eval_SceneFlow_KITTI_Train_MV(nn.Module):
    def __init__(self, args):
        super(Eval_SceneFlow_KITTI_Train_MV, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        ##  my
        gt_disp_noc = target_dict['target_disp_noc']
        gt_disp_noc_mask = (target_dict['target_disp_mask_noc']==1).float()
        d_valid_epe = _elementwise_epe(out_disp_l1, gt_disp_noc) * gt_disp_noc_mask
        d_outlier_epe = (d_valid_epe > 3).float() * ((d_valid_epe / gt_disp) > 0.05).float() * gt_disp_noc_mask
        loss_dict["d1n"] = (d_outlier_epe.view(batch_size, -1).sum(1)).mean() / 90185.615    #  18037123.0 / 200 = 90185.615
        d0_outlier_image_noc = d_outlier_epe
        gt_disp_ooc_mask = gt_disp_mask   - gt_disp_noc_mask
        if (gt_disp_ooc_mask == 0).all():  # occ_erea_mask全为零
            loss_dict["d1o"] = torch.tensor(0.0, device=gt_disp.device)
            d0_outlier_image_occ = torch.zeros_like(d_outlier_epe)
        else:
            d_valid_epe = _elementwise_epe(out_disp_l1, gt_disp) * gt_disp_ooc_mask
            d_outlier_epe = (d_valid_epe > 3).float() * ((d_valid_epe / gt_disp) > 0.05).float() * gt_disp_ooc_mask
            loss_dict["d1o"] = (d_outlier_epe.view(batch_size, -1).sum(1)).mean() / 1690.065   #  338013.0 / 200 = 1690.065
            d0_outlier_image_occ = d_outlier_epe

        ##################################################
        ## Optical Flow Eval
        ##################################################
        if 'pwc' in output_dict:
            out_sceneflow = interpolate2d_as(output_dict['flows_f12_pp'][0], gt_flow, mode="bilinear")
        else:
            out_sceneflow = interpolate2d_as(output_dict['flows_f12_pp'][-1], gt_flow, mode="bilinear")
        out_flow, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], out_sceneflow, output_dict["out_disp_l_pp"])
        ...
        ##  my
        gt_flow_noc = target_dict['target_flow_noc']
        gt_flow_noc_mask = (target_dict['target_flow_mask_noc']==1).float()
        f_valid_epe = _elementwise_epe(out_flow, gt_flow_noc) * gt_flow_noc_mask
        f_outlier_epe = (f_valid_epe > 3).float() * ((f_valid_epe / flow_gt_mag) > 0.05).float() * gt_flow_noc_mask
        loss_dict["fn"] = (f_outlier_epe.view(batch_size, -1).sum(1)).mean() / 79331.155   #  15866231.0 / 200 = 79331.155
        flow_outlier_epe_noc = f_outlier_epe
        gt_flow_ooc_mask = gt_flow_mask   - gt_flow_noc_mask
        if (gt_flow_ooc_mask == 0).all():  # occ_erea_mask全为零
            loss_dict["fo"] = torch.tensor(0.0, device=gt_flow.device)
            flow_outlier_epe_occ = torch.zeros_like(f_outlier_epe)
        else:
            f_valid_epe = _elementwise_epe(out_flow, gt_flow) * gt_flow_ooc_mask
            f_outlier_epe = (f_valid_epe > 3).float() * ((f_valid_epe / flow_gt_mag) > 0.05).float() * gt_flow_ooc_mask
            loss_dict["fo"] = (f_outlier_epe.view(batch_size, -1).sum(1)).mean() / 12544.525   #  2508905.0 / 200 = 12544.525
            flow_outlier_epe_occ = f_outlier_epe
        ...
        ##  my
        gt_disp2_noc = target_dict['target_disp2_noc']
        gt_disp2_noc_mask = (target_dict['target_disp2_mask_noc']==1).float()
        d_valid_epe = _elementwise_epe(out_disp_l1_next, gt_disp2_noc) * gt_disp2_noc_mask
        d_outlier_epe = (d_valid_epe > 3).float() * ((d_valid_epe / gt_disp2_occ) > 0.05).float() * gt_disp2_noc_mask
        loss_dict["d2n"] = (d_outlier_epe.view(batch_size, -1).sum(1)).mean() / 77419.435   #  15483887.0 / 200 = 77419.435
        d1_outlier_image_noc = d_outlier_epe
        gt_disp2_ooc_mask = gt_disp2_mask   - gt_disp2_noc_mask
        if (gt_disp2_ooc_mask == 0).all():  # occ_erea_mask全为零
            loss_dict["d2o"] = torch.tensor(0.0, device=gt_disp2_occ.device)
            d1_outlier_image_occ = torch.zeros_like(d_outlier_epe)  
        else:
            d_valid_epe = _elementwise_epe(out_disp_l1_next, gt_disp2_occ) * gt_disp2_ooc_mask
            d_outlier_epe = (d_valid_epe > 3).float() * ((d_valid_epe / gt_disp2_occ) > 0.05).float() * gt_disp2_ooc_mask
            loss_dict["d2o"] = (d_outlier_epe.view(batch_size, -1).sum(1)).mean() / 14453.955    #  2890791.0 / 200 = 14453.955
            d1_outlier_image_occ = d_outlier_epe
        ...
        outlier_sf_noc = (flow_outlier_epe_noc.bool() + d0_outlier_image_noc.bool() + d1_outlier_image_noc.bool()).float() * \
                            (gt_disp_noc_mask * gt_disp2_noc_mask * gt_flow_noc_mask)
        loss_dict["sn"] = (outlier_sf_noc.view(batch_size, -1).sum(1)).mean() / 77419.435    #   15483887.0 / 200 = 77419.435
        outlier_sf_occ = (flow_outlier_epe_occ.bool() + d0_outlier_image_occ.bool() + d1_outlier_image_occ.bool()).float() * \
                            (gt_disp_ooc_mask.bool() + gt_disp2_ooc_mask.bool() + gt_flow_ooc_mask.bool())
        loss_dict["so"] = (outlier_sf_occ.view(batch_size, -1).sum(1)).mean() / 14456.245   #  2891249 / 200 = 14456.245

        # tensor_a = torch.ones((2, 2))
        # tensor_b = torch.zeros((2, 2))
        # tensor_c = torch.zeros((2, 2))

        # tensor_abc = (tensor_a.bool() + tensor_b.bool() + tensor_c.bool())
        # print(tensor_abc)
        ...
        # # ##########################################
        # #  Optical Flow
        # out_sceneflow_b = interpolate2d_as(output_dict['flows_b21_pp'][-1], gt_flow, mode="bilinear")
        # out_flow_b, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], out_sceneflow_b, output_dict["out_disp_l_pp"])
        # output_dict["out_flow_pp_b"] = out_flow_b
        # #  Depth 2
        # out_depth_l1_next_b = out_depth_l1 + out_sceneflow_b[:, 2:3, :, :]
        # out_disp_l1_next_b = _depth2disp_kitti_K(out_depth_l1_next_b, intrinsics[:, 0, 0])
        # output_dict["out_disp_l_pp_next_b"] = out_disp_l1_next_b
...
class Eval_SceneFlow_KITTI_Train_Mono(nn.Module):
    def __init__(self, args):
        super(Eval_SceneFlow_KITTI_Train_Mono, self).__init__()
...
class Loss_SceneFlow_SelfSup_NoOcc(nn.Module):
    ...
    def sceneflow_loss(self, sf_f, sf_b, disp_l1, disp_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
        ...
        pts2_warp, _ = reconstructPts(coord1, pts2)
        pts1_warp, _ = reconstructPts(coord2, pts1) 
